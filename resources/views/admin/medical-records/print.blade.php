...
<head>
    <meta charset="utf-8" />
    <title>{{ __('medical_records.print_title') }} - PrimaCare</title>
...
<body onload="window.print()">
    ...
    <div class="report-header">
        ...
        <h1>{{ __('medical_records.print_title') }}</h1>
        <div class="report-meta">
            <span>{{ __('medical_records.print_date') }}: {{ now()->format('Y-m-d H:i') }}</span>
        </div>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $record->phone ?? '-' }}</td>
                <td>{{ $record->gender ? __('medical_records.' . $record->gender) : '—' }}</td>
                <td>{{ $record->occupation ?? '—' }}</td>
                <td class="date-col">{{ $record->date_of_birth ? $record->date_of_birth->format('Y-m-d') : '—' }}</td>
                <td class="notes-col">{{ $record->notes ?? '-' }}</td>
                <td>{{ $record->creator->name ?? '-' }}</td>
                <td class="date-col">{{ $record->created_at->format('Y-m-d') }}</td>
            </tr>
            @empty
            <tr>
                <td colspan="11" style="text-align:center; padding:20px;">{{ __('medical_records.no_results') }}</td>
            </tr>
            @endforelse
        </tbody>
    </table>
    ...
    <div class="total-count">
        {{ __('medical_records.total_records') }}: {{ $records->count() }}
    </div>
